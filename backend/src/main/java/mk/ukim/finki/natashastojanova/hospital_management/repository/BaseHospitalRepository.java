...
public interface BaseHospitalRepository extends JpaSpecificationRepository<BaseHospital> {
    ...
    @Query(value = "INSERT INTO base_hospital (id,name,description) VALUES (nextval(main_sequence),:name,:description) RETURNING *", nativeQuery = true)
    public BaseHospital addNewBaseHospital(@Param("name") String name, @Param("description") String description);

    @Query(value = "INSERT INTO Hospital (id,location,id_base_hospital) VALUES (7,:location,:id) RETURNING *", nativeQuery = true)
